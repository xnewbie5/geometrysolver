...
class problem2:
    def __init__(self):
        ...
        self.triangleAreaPartialLarger= None
    ...
    def set_Example(self):
        ...
        self.set_vertex("center", sympify("4",rational=True) ,sympify("0.6",rational=True))
        self.List["radius"] = sympify("0.6",rational=True)
    ...
    def clear(self):
        ...
        self.triangleArea = None
        self.circleAreaInTriangle = None
        self.circleAreaInTriangle2 = None
        self.circleAreaOutsideTriangle = None
        self.triangleAreaPartial = None
    ...
    def print_extra_variables(self):
        ...
        if(isinstance(self.i2,Point2D)):
            ...
            print("i2: ", None)
        if(isinstance(self.i3,Point2D)):
            print("i3: ", self.i3.evalf())
        else:
            print("i3: ", None)
        print("c1: ", self.c1)
        print("c2: ", self.c2)
        ...
        if(self.triangleAreaPartial != None):
            print("Small Triangle Area: ", round(self.triangleAreaPartial,2))
        else:
            print("Small Triangle Area: ", None)
        if(self.triangleAreaPartialLarger != None):
            print("Larger Triangle Area: ", round(self.triangleAreaPartialLarger,2))
        else:
            print("Larger Triangle Area: ", None)
    ...
    def solve_extra_variables(self):
        ...
        if(self.i1 != None and self.i2 != None and self.i3 != None):
            smallTri = Triangle(self.i1, self.i2, self.List["center"])
            smallTri2= Triangle(self.i3, self.i2, self.List["center"])
            #smallTri must NOT be a segment (aka center is collinear with i1 and i2.)
            if((not isinstance(smallTri, Segment2D)) and isinstance(smallTri, Triangle) and
                (not isinstance(smallTri2, Segment2D)) and isinstance(smallTri2, Triangle)):
                ray1 = Ray(self.List["center"],self.i1)
                ray2 = Ray(self.List["center"],self.i2)
                ray3 = Ray(self.List["center"],self.i3) #the i3 thats tangent
                #returns the nonreflex angle (smaller) in radians
                c2andc3 = self.toDegrees(ray1.angle_between(ray2))
                self.c1 = 360-c2andc3
                #print("c1: ",self.c1)
                #print("c2andc3: ", c2andc3)
                #swap them if the circle is inside.
                if(tTri.encloses_point(self.List["center"])):
                    #print("PERFORMING SWAP")
                    temp = c2andc3
                    c2andc3 = self.c1
                    self.c1 = temp
                    #print("NEW c1: ",self.c1)
                    #print("NEW c2andc3: ", c2andc3)
                    #now,c2andc3 is the bigger angle.
                self.c3 = self.toDegrees(ray3.angle_between(ray2))
                #print("c3: ", self.c3)
                self.c2 = c2andc3 - self.c3
                #print("c2: ", self.c2)
                
                if(tTri.encloses_point(self.List["center"])):
                    self.circleAreaInTriangle = float((c2andc3 /360)*float(self.circleArea))
                else:
                    self.circleAreaInTriangle = float((c2andc3 /360)*float(self.circleArea)) - float(smallTri.area)
                smallAreai2i3c3 = float((self.c3 /360)*float(self.circleArea)) - float(smallTri2.area)
                #print("smallAreai2i3c3", smallAreai2i3c3)
                tTri2 = Triangle(self.i3,self.i2,self.List["v3"])
                #print("tTri2 Area: ", abs(float(tTri2.area)))
                self.triangleAreaPartial = round( abs(float(tTri2.area)) - float(smallAreai2i3c3), 2 )
                #print("triangle area partial: ", self.triangleAreaPartial)
                self.triangleAreaPartialLarger = round((self.triangleArea - self.triangleAreaPartial - self.circleAreaInTriangle), 2)
                #print("triangle area partial larger: ", self.triangleAreaPartialLarger)
    ...
    def solve_Circle(self):
        ...
        if((tRadius != None) and (isinstance(tCenter, Point2D))):
            ...
            #bad circle. Have the user re-enter everything
                ...
                return True
        #User doesn't enter a center and radius. Solve for where the circle should be (one try)
        elif((not (self.List["radius"] != None)) and (not isinstance(tCenter, Point2D))):
            print("Setting position for circle...")
            tV1 = self.List["v1"]
            tV2 = self.List["v2"]
            tV3 = self.List["v3"]
            tTri = Triangle(tV1, tV2, tV3)
            tTriMedial = tTri.medial
            bigS = Segment(tV2,tV3)
            sMidpoint = tTriMedial.sides[1].midpoint
            self.List["radius"] = bigS.distance(sMidpoint)
            tRadius2 = self.List["radius"]
            sMidpointx = sMidpoint.x
            sMidpointy = sMidpoint.y
            self.set_vertex("center", sMidpointx, sMidpointy)
            tCircle2 = Circle(Point(sMidpointx,sMidpointy), tRadius2)
            print("CHECKING TEMP CIRCLE CREATED WITH JUST RADIUS", tCircle2)
            if(self.checkCircle(tCircle2)==False):
                print("Can not find a suitable circle.")
                self.List["center"] = None
                self.List["radius"] = None
                return True
    ...
    def checkCircle(self, c):
        ...
        if (isinstance(tV1, Point2D) and isinstance(tV2, Point2D) and isinstance(tV3, Point2D)):
            tSegment = Segment(tV1, tV3) #right edge
            tSegment2 = Segment(tV1, tV2) #left edge
            tSegment3 = Segment(tV2, tV3) #bottom edge
        else: 
            return False
        ...
        if((tRadius != None) and (isinstance(tCenter, Point2D))):
            ...
            #good circle, keep
            if( (len(tList)==0 and len(tList1)==1 and len(tList2)==2 ) or 
                (len(tList)==0 and len(tList1)==2 and len(tList2)==1 ) or
                (len(tList)==1 and len(tList1)==0 and len(tList2)==2 ) or
                (len(tList)==1 and len(tList1)==2 and len(tList2)==0 ) or 
                (len(tList)==2 and len(tList1)==0 and len(tList2)==1 ) or
                (len(tList)==2 and len(tList1)==1 and len(tList2)==0 )  ):
                if(len(tList)==2):
                    #print(tList)
                ...
                print("Error. Doesn't fit constraints.")
                return False
