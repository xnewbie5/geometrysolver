...
class problem1:
    ...
    def solve_Circle(self):
        ...
        if((tRadius != None) and (isinstance(tCenter, Point2D))):
            ...
            #bad circle. Have the user re-enter everything
                ...
                print("Bad Circle. Try another circle? (Try entering either center or radius)...")
                return True
        #User enters a radius but not center. Solve for center
        elif((self.List["radius"] != None) and (not isinstance(tCenter, Point2D))):
            pass
        #User enters a center, but not radius. Solve for radius
        return True
    ...
    def checkCircle(self, c):
        ...
        if((tRadius != None) and (isinstance(tCenter, Point2D))):
            ...
            tList1 = tCircle.intersection(tSegment2)
            tList2 = tCircle.intersection(tSegment3)
            #good circle, keep
